...
- 办公室 Mac 可以访问 `http://8.134.145.9:8080`
...
```powershell
ssh -N -L 8080:8.134.145.9:8080 choco@100.105.189.124
```

也可以使用 MagicDNS：

```powershell
ssh -N -L 8080:8.134.145.9:8080 choco@chocos-mac.tail2e7a8d.ts.net
```
...
```powershell
ssh -N -L 18080:8.134.145.9:8080 choco@100.105.189.124
```
...
- SSH 隧道只转发 `127.0.0.1:8080 -> 8.134.145.9:8080`，不会代理所有网络
